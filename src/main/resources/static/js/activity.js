$('#activityListTable').bootstrapTable({
        url: '/adminpioneer/admin/activityList',   //请求后台的URL（*）
        method: 'post',                      //请求方式（*）
        ...
        columns: [{
            ...
            formatter: function (value, row, index) {//操作栏的格式化
                var id = value;
                var result = "";
                result += "<a href='/adminpioneer/admin/activity/" + id + "' class='btn btn-xs'  title='编辑'><span class='glyphicon glyphicon-pencil'></span></a>";
...
function deleteById(id) {
    if (confirm("确定要删除吗？")) {
        $.ajax({
            type: "post",
            url: "/adminpioneer/admin/activityList/deleteById",
...
$("#btn_delete").click(function () {
    var rows = $("#activityListTable").bootstrapTable('getSelections');
    if (rows.length == 0) {
        alert("请选择要删除的记录");
        return;
    }
    if (confirm("确定要删除吗？")) {
        ...
        $.ajax({
            type: "post",
            url: "/adminpioneer/admin/activityList/deleteByIds",
            contentType: 'application/json;charset=utf-8',  //发送信息至服务器时内容编码类型
